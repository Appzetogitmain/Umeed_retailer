...
export default function AdminProcessedOrders() {
  ...
  const handleExportExcel = () => {
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `processed_orders_${new Date().toISOString().split('T')[0]}.xls`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text('Speedoo - Processed Orders Export', 14, 15);
    doc.setFontSize(10);
    ...
    doc.save(`processed_orders_${new Date().toISOString().split('T')[0]}.pdf`);
  };
