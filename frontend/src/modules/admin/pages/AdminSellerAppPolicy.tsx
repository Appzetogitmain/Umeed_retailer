...
type PolicyType = "seller_privacy_policy" | "seller_terms_and_conditions";

export default function AdminSellerAppPolicy() {
  const [activeTab, setActiveTab] = useState<PolicyType>("seller_privacy_policy");
  ...
  const getDefaultTitle = (type: PolicyType) => {
    switch (type) {
      case 'seller_privacy_policy': return 'Seller Privacy Policy';
      case 'seller_terms_and_conditions': return 'Seller Terms and Conditions';
      default: return '';
    }
  };
    ...
    <div className="flex flex-col h-full bg-neutral-50">
      ...
      <div className="bg-white px-4 sm:px-6 py-4 border-b border-neutral-200 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-neutral-900">Seller App Content & Policies</h1>
          </div>
            <p className="text-sm text-neutral-500 mt-1">Manage what sellers see in the Legal sections</p>
          </div>
          <div className="text-sm">
            <span className="text-teal-600 font-medium">Admin</span> / <span className="text-neutral-400">Seller Content</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white px-4 sm:px-6 border-b border-neutral-200">
        <div className="flex space-x-8 overflow-x-auto no-scrollbar">
          {(['seller_privacy_policy', 'seller_terms_and_conditions'] as PolicyType[]).map((type) => (
            <button
              key={type}
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-neutral-200 overflow-hidden">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        required
                        className="w-full px-4 py-2 border border-neutral-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-neutral-800 mb-2">
              ...
              <div className="bg-neutral-50 px-6 py-4 flex justify-end">
                <button
                  type="submit"
                  disabled={loading}
                  className="px-8 py-3 bg-teal-600 text-white rounded-xl font-bold shadow-lg shadow-teal-200 hover:bg-teal-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  {loading ? (
                    <>
