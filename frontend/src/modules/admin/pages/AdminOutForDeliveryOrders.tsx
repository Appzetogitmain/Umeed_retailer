...
export default function AdminOutForDeliveryOrders() {
  ...
  useEffect(() => {
    ...
    fetchOrders();
  }, [isAuthenticated, token, currentPage, entriesPerPage, searchQuery, dateFrom, dateTo, seller]);
  ...
  const handleExportExcel = () => {
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `out_for_delivery_orders_${new Date().toISOString().split('T')[0]}.xls`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text('Speedoo - Out for Delivery Orders Export', 14, 15);
    doc.setFontSize(10);
    ...
    doc.save(`out_for_delivery_orders_${new Date().toISOString().split('T')[0]}.pdf`);
  };
