...
export default function AdminDeliveredOrders() {
  ...
  const handleExportExcel = () => {
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `delivered_orders_${new Date().toISOString().split('T')[0]}.xls`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text('Speedoo - Delivered Orders Export', 14, 15);
    doc.setFontSize(10);
    ...
    doc.save(`delivered_orders_${new Date().toISOString().split('T')[0]}.pdf`);
  };
