...
export default function DeliveryTermsAndConditions() {
  ...
  useEffect(() => {
    const fetchPolicy = async () => {
      try {
        const response = await getPolicyByType('delivery_terms_and_conditions');
        if (response.success) {
          setPolicy(response.data);
        }
      } catch (err) {
        console.error('Failed to fetch delivery terms and conditions:', err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-neutral-200">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-neutral-900 leading-tight">Delivery Partner Terms & Conditions</h1>
          ...
          {policy && (
            <div className="hidden sm:flex flex-col items-end">
              ...
              <span className="text-xs font-bold text-blue-600">{policy.version}</span>
            </div>
          )}
        </div>
      </div>
              ...
              <div className="mt-16 pt-8 border-t border-neutral-100">
                <div className="bg-neutral-50 rounded-2xl p-6 text-center">
                  <p className="text-sm text-neutral-500 italic mb-4">
                    If you have any questions regarding these terms, please contact our delivery support team.
                  </p>
